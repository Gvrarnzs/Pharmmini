...
import AddMedin from '../../Component/AddMedicinein';
import EditmodalMedicine from '../../Component/EditMedicinein';
import { useRouter } from 'next/router'


const ObatMasuk = () => {
    const colRef = collection(db, 'medin')
    ...
    const realtimeFunc = () => {
        ...
        onSnapshot(colRef, (snapshot) => {
            ...
            snapshot.docs.forEach((doc) => {
                medIndata.push({...doc.data(), id: doc.id})
            })
            setdataMasuk(medIndata)
            setloading(false)
            console.log(medIndata)
        })
    }
    useEffect(() => {
        realtimeFunc()
    ...
    const deleteUsers = (index) => {
        Swal.fire({
            title: `Are you sure to delete ${dataMasuk[index].name}?`,
            ...
            if (result.isConfirmed) {
                const userDoc = doc(db, "medin",dataMasuk[index].id )
                deleteDoc(userDoc)
                Swal.fire(
                    'Deleted!',
    ...
    return (
        <div className='bg-primary min-h-screen'>
            <Header/>
            <EditmodalMedicine openEdit={openEdit} handleModalEdit={handleModalEdit} dataEditin={dataMasuk[indexEdit]} />
            <AddMedin open={open} handleModal={handleModal} />
            <div className='flex justify-between pr-5 pt-6 pl-5'>
                <div className='text-secondary text-2xl'>Medicine - In</div>
